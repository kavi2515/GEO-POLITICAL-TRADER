...
function aggregateAssets(signals: SignalItem[]): { buys: AssetSummary[]; sells: AssetSummary[] } {
  const map = new Map<string, { buy: number[]; sell: number[]; label: string; category: string; topSignalId: string; topNewsTitle: string; sources: AssetSummary["sources"] }>();

  for (const signal of signals) {
    for (const ms of signal.market_signals) {
      const key = ms.asset;
      if (!map.has(key)) map.set(key, { buy: [], sell: [], label: ms.asset_label, category: ms.category, topSignalId: signal.id, topNewsTitle: signal.news_title, sources: [] });
  ...
  for (const [asset, data] of map.entries()) {
    ...
    if (data.buy.length > data.sell.length || (data.buy.length === data.sell.length && buyAvg >= sellAvg)) {
      buys.push({ asset, asset_label: data.label, category: data.category, direction: "BUY", avgConfidence: Math.round(buyAvg), count: data.buy.length, topSignalId: data.topSignalId, topNewsTitle: data.topNewsTitle, sources: data.sources });
    } else {
      sells.push({ asset, asset_label: data.label, category: data.category, direction: "SELL", avgConfidence: Math.round(sellAvg), count: data.sell.length, topSignalId: data.topSignalId, topNewsTitle: data.topNewsTitle, sources: data.sources });
    }
  }
